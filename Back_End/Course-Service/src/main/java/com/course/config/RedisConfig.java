    ...
    public RedisCacheManager manager(RedisConnectionFactory redisConnectionFactory){
        ...
        RedisCacheConfiguration singleCache  =   RedisCacheConfiguration.defaultCacheConfig()
                ...
                .serializeValuesWith(RedisSerializationContext.SerializationPair
                        .fromSerializer(new Jackson2JsonRedisSerializer<>(ResponseCourseDTO.class)));


        //cache for list of object
        ObjectMapper objectMapper = new ObjectMapper();
        //JavaType is a type to store  List<T> and so on in a serilzation and deserilization
        JavaType type = objectMapper.getTypeFactory().constructCollectionType(List.class, ResponseCourseDTO.class);
        Jackson2JsonRedisSerializer<List<ResponseCourseDTO>> listTYpe = new Jackson2JsonRedisSerializer<>(type);
